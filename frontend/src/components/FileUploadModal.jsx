                ...
                {uploadResult.duplicates_skipped > 0 && (
                  <div className="p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm text-yellow-800 font-medium">Duplicates Skipped:</span>
                      <span className="font-semibold text-yellow-800">{uploadResult.duplicates_skipped}</span>
                    </div>
                    <p className="text-xs text-yellow-700">
                      These transactions already exist and were not added again. This prevents duplicate entries when uploading the same PDF multiple times.
                    </p>
                  </div>
                )}

                {uploadResult.duplicates_skipped === 0 && uploadResult.new_transactions_added > 0 && (
                  <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                    <div className="flex items-center gap-2">
                      <CheckCircle className="h-4 w-4 text-green-600" />
                      <span className="text-sm text-green-800 font-medium">No Duplicates Found</span>
                    </div>
                    <p className="text-xs text-green-700 mt-1">
                      All transactions from this PDF are new and have been successfully added.
                    </p>
                  </div>
                )}

                {uploadResult.total_transactions_found > 0 && uploadResult.new_transactions_added === 0 && (
                  <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
                    <div className="flex items-center gap-2">
                      <AlertCircle className="h-4 w-4 text-orange-600" />
                      <span className="text-sm text-orange-800 font-medium">All Transactions Were Duplicates</span>
                    </div>
                    <p className="text-xs text-orange-700 mt-1">
                      This PDF has been uploaded before. All {uploadResult.total_transactions_found} transactions already exist in your account.
                    </p>
                  </div>
                )}
              </div>
            ...
            <div className="space-y-4">
              ...
              <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                <div className="flex items-center gap-2 mb-2">
                  <CheckCircle className="h-4 w-4 text-green-600" />
                  <p className="text-sm font-medium text-green-900">Duplicate Protection</p>
                </div>
                <p className="text-xs text-green-700">
                  Don't worry about uploading the same PDF twice! We automatically detect and skip duplicate transactions based on date, amount, and description.
                </p>
              </div>
